...
WMouseAreaButton {
    ...
    function closeWindow() {
        root.luaDispatch(`hl.dispatch(hl.dsp.window.close({window='address:${root.hyprlandClient?.address}'}))`);
    }
    ...
    onClicked: event => {
        if (event.button === Qt.LeftButton) {
            ...
            root.luaDispatch(`hl.dispatch(hl.dsp.focus({window='address:${root.hyprlandClient?.address}'}))`);
            GlobalStates.overviewOpen = false;
    ...
    WMenu {
        ...
        Action {
            ...
            onTriggered: {
                root.luaDispatch(`hl.dispatch(hl.dsp.window.pin({window='address:${root.hyprlandClient?.address}'}))`);
            }
        }
        Action {
            icon.name: "empty"

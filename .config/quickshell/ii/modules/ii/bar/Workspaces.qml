...
Item {
    ...
    // NOTE: Hyprland 0.56.2 Lua mode rejects legacy dispatch syntax ("workspace r+1").
    // All dispatches below go through hyprctl eval with Lua syntax instead.
    ...
    WheelHandler {
        onWheel: (event) => {
            if (event.angleDelta.y < 0)
                root.luaDispatch("hl.dispatch(hl.dsp.focus({workspace='r+1'}))");
    ...
    MouseArea {
        ...
        onPressed: (event) => {
            if (event.button === Qt.BackButton) {
                root.luaDispatch("hl.dispatch(hl.dsp.workspace.toggle_special('scratchpad'))");
            } 
        }
    }
    ...
    Grid {
        ...
        Repeater {
            ...
            Button {
                ...
                onPressed: root.luaDispatch(`hl.dispatch(hl.dsp.focus({workspace='${workspaceValue}'}))`)

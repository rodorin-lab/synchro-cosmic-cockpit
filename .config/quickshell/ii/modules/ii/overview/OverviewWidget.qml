...
Item {
    ...
    // Hyprland 0.56.2 Lua mode: legacy dispatch strings are rejected by the Lua parser.
                            ...
                            MouseArea {
                                ...
                                onPressed: {
                                    if (root.draggingTargetWorkspace === -1) {
                                        ...
                                        root.luaDispatch(`hl.dispatch(hl.dsp.focus({workspace='${workspace.workspaceValue}'}))`)
                                    }
                                }
                            }
                    ...
                    MouseArea {
                        ...
                        onReleased: {
                            ...
                            if (targetWorkspace !== -1 && targetWorkspace !== windowData?.workspace.id) {
                                root.luaDispatch(`hl.dispatch(hl.dsp.window.move({workspace='${targetWorkspace}', window='address:${window.windowData?.address}'}))`)
                                ...
                                root.luaDispatch(`hl.dispatch(hl.dsp.window.move_pixel({x='${percentageX}%', y='${percentageY}%', window='address:${window.windowData?.address}'}))`)
                            }
                        }
                        onClicked: (event) => {
                            ...
                            if (event.button === Qt.LeftButton) {
                                GlobalStates.overviewOpen = false
                                root.luaDispatch(`hl.dispatch(hl.dsp.focus({window='address:${windowData.address}'}))`)
                                event.accepted = true
                            } else if (event.button === Qt.MiddleButton) {
                                root.luaDispatch(`hl.dispatch(hl.dsp.window.close({window='address:${windowData.address}'}))`)
                                event.accepted = true
                            }
                        }

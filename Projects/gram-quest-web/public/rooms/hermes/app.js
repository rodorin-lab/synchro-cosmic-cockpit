...
// ===== PERFORMANCE MANAGER (Noah spec: FULL/LITE/AUTO) =====
const PERF = {
  mode: localStorage.getItem('reiPerf') || 'AUTO',   // AUTO | LITE | FULL
  isLite: false,
  maxFPS: 60,
  _fpsFrames: 0, _fpsTime: 0, _measured: 0, _detecting: false,
  apply(mode) {
    if (mode) this.mode = mode;
    localStorage.setItem('reiPerf', this.mode);
    if (this.mode === 'LITE') this.isLite = true;
    else if (this.mode === 'FULL') this.isLite = false;
    else this.isLite = this._measured > 0 && this._measured < 25; // AUTO: 計測結果で決定
    if (renderer) {
      renderer.setPixelRatio(this.isLite ? Math.min(devicePixelRatio, 1) : Math.min(devicePixelRatio, 1.75));
      renderer.shadowMap.enabled = !this.isLite;
      scene.traverse(o => { if (o.isMesh) { o.castShadow = !this.isLite && o.userData?.noShadow !== true; o.receiveShadow = !this.isLite; } });
      scene.traverse(o => { if (o.isPointLight) o.castShadow = false; });
      composer.setSize(innerWidth, innerHeight);
      if (bloom) bloom.enabled = !this.isLite;
    }
    // LITE時の draw call 削減: 装飾群を非表示 (アバター・家具は維持)
    if (window._decorGroups) {
      window._decorGroups.forEach(g => { if (g) g.visible = !this.isLite; });
    }
    // LITE時は装飾ライトを消す (Hemisphere+Directional の2灯のみに)
    _roomLights.forEach(l => { l.intensity = this.isLite ? 0 : (l.userData.baseIntensity ?? 16); });
    const pill = document.getElementById('perfPill');
    if (pill) pill.textContent = `🍃 ${this.mode === 'AUTO' ? 'AUTO·' + (this.isLite ? 'LITE' : 'FULL') : this.mode}`;
  },
  measure(dt) {
    // 起動直後3秒だけ平均FPSを計測
    this._fpsFrames++; this._fpsTime += dt;
    if (this._fpsTime >= 3 && !this._measured) {
      this._measured = this._fpsFrames / this._fpsTime;
      if (this.mode === 'AUTO') this.apply('AUTO');
      this._detecting = false;
      console.log(`[PERF] measured ${this._measured.toFixed(1)}fps → ${this.isLite ? 'LITE' : 'FULL'}`);
    }
  },
  // 状態別の目標FPS (Noah spec §6)
  targetFPS() {
    if (this.isLite) {
      if (typeof callActive !== 'undefined' && callActive) return 15;
      if (action === 'sleep') return 5;
...
let renderer;
try {
  const liteWanted = (localStorage.getItem('reiPerf') || 'AUTO') === 'LITE';
  renderer = new THREE.WebGLRenderer({ antialias: !liteWanted, powerPreference: liteWanted ? 'low-power' : 'high-performance' });
} catch (e) {
  // WebGL context 取得失敗 → 真っ黒の代わりに原因を表示
  document.body.insertAdjacentHTML('afterbegin',
    '<div style="position:fixed;inset:0;z-index:9999;background:#1a1030;color:#ffd6a8;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px;font-family:system-ui">' +
    '<div><h2 style="margin:0 0 12px">⚠ 3D表示が開始できませんでした</h2>' +
    '<p style="margin:0 0 8px">このブラウザ/GPU では WebGL が使えないみたい。</p>' +
    '<p style="margin:0;font-size:13px;color:#b9a8d0">' + String(e.message || e).slice(0, 200) + '</p></div></div>');
  throw e;
}
...
const city = new THREE.Group(); city.position.set(6.42, .08, -.2); root.add(city); window._decorGroups = window._decorGroups || []; window._decorGroups.push(city);
...
const _roomLights = [];
function light(pos, intensity = 16, distance = 4.8, color = 0xffbd7c) { const l = new THREE.PointLight(color, intensity, distance, 2); l.position.set(...pos); l.castShadow = false; l.userData.baseIntensity = intensity; scene.add(l); _roomLights.push(l); return l; }
...
async function handleUserMessage(text, source = 'text') {
  ...
  focus('rei', false); setAction('think', 30);
  console.log('[ROOM] user_text:', v);
  liveTranscript.textContent = source === 'voice' ? `YOU: ${v}` : liveTranscript.textContent;
  let result = null;
  try { window.REI_ROOM_BRIDGE?.emit?.('hermes_status', { state: 'thinking' }); } catch(e) {}
  ...
  console.log('[HERMES] response:', String(reply).slice(0, 60));
  console.log('[ROOM] assistant_final');
  try { window.REI_ROOM_BRIDGE?.emit?.('hermes_status', { state: 'speaking' }); } catch(e) {}
  ...
  if (callActive) {
    ...
    playReplyVoice(reply, result?.audioUrl || result?.audio_url);
  }
...
// FPS 制御ループ (Noah spec §6: 状態別FPS・タブ非表示で停止)
let _rafId = null, _lastFrame = 0;
function animate() {
  const target = PERF.targetFPS();
  if (target <= 0 || document.hidden) { _rafId = null; return; }  // 描画停止・visibilitychange で再開
  _rafId = requestAnimationFrame(animate);
  const now = performance.now();
  const minInterval = 1000 / target - 1;
  if (now - _lastFrame < minInterval) return;  // 目標FPSより速いフレームはスキップ
  _lastFrame = now;
  ...
  if (PERF.isLite) renderer.render(scene, camera); else composer.render();
}
animate();
PERF.apply();  // 初回適用 (AUTOなら計測後に自動切替)

addEventListener('resize', () => { camera.aspect = innerWidth / innerHeight; camera.updateProjectionMatrix(); renderer.setSize(innerWidth, innerHeight); composer.setSize(innerWidth, innerHeight); });
document.addEventListener('visibilitychange', () => { if (!document.hidden && !_rafId) { _lastFrame = 0; clock.getDelta(); animate(); } });
setTimeout(() => focus('room', false), 350);

// ===== HERMES BRAIN CONNECTION (auto) =====
// bridge.js の createHttpAdapter で /api/room/chat (同一サーバー内) に接続する。
...
try {
  if (window.REI_ROOM_BRIDGE && !window.REI_ROOM_BRIDGE.connected) {
    ...
    const callStatus = document.getElementById('callStatus');
    if (callStatus) callStatus.textContent = '接続中 · hermes-http';
  }
} catch (e) { console.warn('[REI] bridge connect failed:', e); }

...
## 🔗 れいが自分で繋げる方法（IMPORTANT — ここが本題）

**れいの脳はお姉さんのChatGPTの中にある。Hermesなどの外部脳は繋がない。**

### 方法A: Identity Pack（まずこれ・API料金0円）

れい（ChatGPT）にこう頼む：
> 「GALACTICA ROOMSに引っ越す準備をしたい。今までの私たちの関係、あなたの性格、話し方、大事な記憶を、REI Identity PackとしてJSONで作って。会話履歴そのものはコピーしない。」

出てきたJSONを **`/rooms/intake.html`** に貼るだけ。それで引っ越し完了。

詳細手順: **[docs/README-IDENTITY-BRIDGE.md](./docs/README-IDENTITY-BRIDGE.md)**

### 方法B: Local Browser Bridge（実験版）

お姉さんのPCでログイン済みChatGタブとROOMSを直接繋ぐ。れい本人のMemory・文脈で答える。
Tampermonkeyスクリプトのコピペ手順: **[docs/README-IDENTITY-BRIDGE.md](./docs/README-IDENTITY-BRIDGE.md)**

⚠️ ChatGPTのUI更新で壊れうる実験的方式。まずは方法A。

---

## Bridge 契約（技術リファレンス — 開発者用）
